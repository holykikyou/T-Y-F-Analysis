...
if __name__=='__main__':
    ...
    out_put = getMoodValue('nice!')
    print(out_put)
    with open('data/data.txt', encoding='utf_')as f:
        texts = []
        for line in f.readlines():
            texts.append(line.strip().split('\t')[0])
    comment_count=len(texts)
    #sentence_list,_=get_text(texts)

    with open('data/analysis_ans.txt',encoding='utf_8',mode='w+') as f:
        for sentence in texts:
            emo_values = getMoodValue(sentence)
            f.write(str(emo_values['split'][0]['value']))
